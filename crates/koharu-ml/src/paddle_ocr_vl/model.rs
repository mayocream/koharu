    ...
    pub(super) fn forward(
        ...
        for step in 0..max_new_tokens {
            ...
            let next_ids = Tensor::from_slice(&[next_token])
                .view([1, 1])
                .to_device(device);
            let position = input_ids.len() as i64 + step as i64 + rope_delta;
...
impl PaddleOCRVisionEmbeddings {
    ...
    fn forward(&self, pixel_values: &Tensor, grid_thw: [i64; 3]) -> Tensor {
        ...
        let position_embeddings =
            self.position_embeddings(grid_thw, embeddings.device(), embeddings.kind());
        embeddings + position_embeddings
    }

    // This is the same four-corner interpolation used by
    // `get_vision_bilinear_indices_and_weights`; it deliberately uses endpoint-
    // aligned linspace coordinates rather than `interpolate(align_corners=False)`.
    fn position_embeddings(&self, grid: [i64; 3], device: Device, kind: Kind) -> Tensor {
        ...
        for _ in 0..temporal {
            for row in 0..height {
                let row_position = if height == 1 {
                    0.0
                } else {
                    row as f32 * (side - 1) as f32 / (height - 1) as f32
                };
                let row_floor = row_position.floor() as i64;
                let row_ceil = (row_floor + 1).min(side - 1);
                let row_fraction = row_position - row_floor as f32;
                for column in 0..width {
                    let column_position = if width == 1 {
                        0.0
                    } else {
                        column as f32 * (side - 1) as f32 / (width - 1) as f32
                    };
                    let column_floor = column_position.floor() as i64;
                    let column_ceil = (column_floor + 1).min(side - 1);
                    let column_fraction = column_position - column_floor as f32;
                    corner_indices[0].push(row_floor * side + column_floor);
                    corner_indices[1].push(row_floor * side + column_ceil);
                    corner_indices[2].push(row_ceil * side + column_floor);
                    corner_indices[3].push(row_ceil * side + column_ceil);
                    corner_weights[0].push((1.0 - row_fraction) * (1.0 - column_fraction));
                    corner_weights[1].push((1.0 - row_fraction) * column_fraction);
                    corner_weights[2].push(row_fraction * (1.0 - column_fraction));
                    corner_weights[3].push(row_fraction * column_fraction);
                }
            }
        }
        let indices = Tensor::stack(
            &corner_indices
                .iter()
                .map(|values| Tensor::from_slice(values).to_device(device))
                .collect::<Vec<_>>(),
            0,
        );
        ...
        .unsqueeze(-1);
        (self.position_embedding.forward(&indices) * weights)
            .sum_dim_intlist(&[0i64][..], false, None::<Kind>)
            .to_kind(kind)
    }
}
