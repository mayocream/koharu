...
pub(super) struct Yolo11nSpeechBubbleProcessor {
    model: Arc<Mutex<Yolo11nSpeechBubbleSegmenter>>,
    config: Yolo11nSpeechBubbleConfig,
}

impl Yolo11nSpeechBubbleProcessor {
    pub(super) async fn load(
        ...
        Ok(Self {
            model: Arc::new(Mutex::new(
                Yolo11nSpeechBubbleSegmenter::load(device).await?,
            )),
            config: config.clone(),
        })
    }
...
#[async_trait]
impl Processor for Yolo11nSpeechBubbleProcessor {
...
fn paint_mask(width: u32, height: u32, regions: &[&Yolo11nSpeechBubbleInstance]) -> GrayImage {
