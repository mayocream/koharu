...
use crate::{Context, Processor, SpeechBubbleSegmentationConfig, Stage};

pub(super) struct SpeechBubbleSegmentationProcessor {
    model: Arc<Mutex<SpeechBubbleSegmentation>>,
    config: SpeechBubbleSegmentationConfig,
}

impl SpeechBubbleSegmentationProcessor {
    pub(super) async fn load(
        ...
        if let Some(value) = config.confidence {
            ensure!(
                value.is_finite() && (0.0..=1.0).contains(&value),
                "speech bubble confidence must be between 0 and 1"
            );
        }
        if let Some(value) = config.nms_iou {
            ensure!(
                value.is_finite() && (0.0..=1.0).contains(&value),
                "speech bubble nms_iou must be between 0 and 1"
            );
        }
        Ok(Self {
            model: Arc::new(Mutex::new(SpeechBubbleSegmentation::load(device).await?)),
            config: config.clone(),
        })
    }
}
...
#[async_trait]
impl Processor for SpeechBubbleSegmentationProcessor {
...
fn paint_mask(width: u32, height: u32, regions: &[&SpeechBubbleRegion]) -> GrayImage {
