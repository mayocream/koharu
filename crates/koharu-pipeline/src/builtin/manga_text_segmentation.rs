...
use crate::{Context, MangaTextSegmentationConfig, Processor, Stage};

pub(super) struct MangaTextSegmentationProcessor {
    model: Arc<Mutex<MangaTextSegmentation>>,
    config: MangaTextSegmentationConfig,
}

impl MangaTextSegmentationProcessor {
    pub(super) async fn load(
        ...
        config: &MangaTextSegmentationConfig,
            ...
            "manga text mask threshold must be between 0 and 1"
        );
        if config.max_side == Some(0) {
            bail!("manga text mask max_side must be positive");
        }
        ...
        Ok(Self {
            model: Arc::new(Mutex::new(MangaTextSegmentation::load(device).await?)),
            config: config.clone(),
        })
    }
}
...
#[async_trait]
impl Processor for MangaTextSegmentationProcessor {
        ...
        let outputs = tokio::task::spawn_blocking(move || {
            ...
            inputs
                ...
                .map(|input| {
                    ...
                    let mask = output.process(&MangaTextCleaningOptions {
