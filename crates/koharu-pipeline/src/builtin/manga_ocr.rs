...
impl MangaOcrProcessor {
    pub(super) async fn load(device: koharu_ml::Device, _config: &MangaOcrConfig) -> Result<Self> {
        Ok(Self {
            model: Arc::new(Mutex::new(MangaOcr::load(device).await?)),
        })
    }
}

#[async_trait]
impl Processor for MangaOcrProcessor {
    ...
    async fn run(&mut self, context: &Context) -> Result<koharu_scene::Commands> {
        ...
        let outputs = tokio::task::spawn_blocking(move || {
            let model = model
                .lock()
                .map_err(|_| anyhow!("manga OCR model lock is poisoned"))?;
            inputs
                .into_iter()
                .map(|input| {
