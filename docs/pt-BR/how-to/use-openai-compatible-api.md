...
Existe uma única configuração de provedor `OpenAI Compatible`. Para alternar entre servidores compatíveis, mude a base URL e a API key opcional. OpenRouter e LM Studio usam entradas de provedor dedicadas.
...
Como existe um único provedor `OpenAI Compatible`, apenas uma base URL personalizada é configurada por vez. OpenRouter e LM Studio permanecem configurados separadamente em seus provedores dedicados.
