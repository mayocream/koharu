...
use koharu_core::{ExportProjectResult, ImportProjectResult};
use anyhow::Result;
use rfd::FileDialog;
...
pub async fn import_project(state: AppResources) -> Result<ImportProjectResult> {
    let Some(input_path) = pick_input_khr().await? else {
    ...
    state.storage.import_khr(&input_path).await?;
...
async fn pick_input_khr() -> Result<Option<std::path::PathBuf>> {
    Ok(tokio::task::spawn_blocking(move || {
        FileDialog::new()
            .add_filter("Koharu Project", &["khr"])
            .pick_file()
