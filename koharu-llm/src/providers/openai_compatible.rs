...
fn normalized_base_url(base_url: &Url) -> String {
...
pub async fn list_models(base_url: &str, api_key: Option<&str>) -> anyhow::Result<Vec<String>> {
    let endpoint = format!("{}/models", normalized_base_url(&base_url.parse::<Url>()?));
...
mod tests {
    ...
    fn trims_trailing_slashes() {
        let normalized = normalized_base_url(&"http://127.0.0.1:1234/v1/".parse::<Url>().unwrap());
