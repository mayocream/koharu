...
use super::mcp::KoharuMcp;
use super::state::SharedResources;
...
pub(crate) type SharedAssetResolver = Arc<dyn Fn(&str) -> Option<Asset> + Send + Sync>;

pub(crate) fn asset_resolver<I>(resolvers: I) -> SharedAssetResolver
where
...
fn build_router(
    ...
    Router::new()
        .nest("/api/v1", api::router(shared.clone(), bootstrap, events))
...
pub(crate) async fn serve_with_listener(
    ...
    let router = build_router(shared, bootstrap, resolver);
