...
use self::types::{
    CreateTextBlockParams, DocumentIndexParams, ExportDocumentParams, InpaintRegionParams,
    LoadModelParams, MaskMorphParams, OpenDocumentsParams, ProcessParams, RemoveTextBlockParams,
    RenderParams, TranslateParams, UpdateTextBlockParams, ViewImageParams, ViewTextBlockParams,
};
...
use koharu_core::{FileEntry, views::to_doc_info};

use crate::server::state::SharedResources;
use crate::services::{
    AppResources, operations,
    request::{
        CreateTextBlockJob, LlmLoadJob, MaskMorphJob, ModelCatalogQuery, PartialInpaintJob,
        PipelineJob, RemoveTextBlockJob, RenderJob, TextBlockUpdate, TranslateJob,
    },
};
...
impl KoharuMcp {
    ...
    async fn device(&self) -> Result<String, String> {
        let res = self.resources()?;
        Ok(operations::device(&res).to_string())
    }
    ...
    async fn get_document(
        &self,
        Parameters(p): Parameters<DocumentIndexParams>,
    ) -> Result<String, String> {
        let res = self.resources()?;
        let doc = operations::get_document(res, p.index)
            .await
            .map_err(|e| e.to_string())?;
    ...
    async fn open_documents(
        ...
        let count = operations::open_documents(res.clone(), files?)
            .await
            .map_err(|e| e.to_string())?;
    ...
    async fn export_document(
        ...
        let res = self.resources()?;
        let data = operations::export_document(res, p.index)
            .await
            .map_err(|e| e.to_string())?;

        std::fs::write(&p.output_path, &data)
    ...
    async fn detect(
        ...
        let res = self.resources()?;
        operations::detect(res.clone(), p.index)
            .await
            .map_err(|e| e.to_string())?;
    ...
    async fn ocr(&self, Parameters(p): Parameters<DocumentIndexParams>) -> Result<String, String> {
        let res = self.resources()?;
        operations::ocr(res.clone(), p.index)
            .await
            .map_err(|e| e.to_string())?;
    ...
    async fn inpaint(
        ...
        let res = self.resources()?;
        operations::inpaint(res, p.index)
            .await
            .map_err(|e| e.to_string())?;
    ...
    async fn llm_load(&self, Parameters(p): Parameters<LoadModelParams>) -> Result<String, String> {
        let res = self.resources()?;
        operations::llm_load(
            res,
            LlmLoadJob {
                ...
                custom_system_prompt: p.custom_system_prompt.clone(),
            },
        )
        .await
        .map_err(|e| e.to_string())?;
    ...
    async fn llm_generate(
        &self,
        Parameters(p): Parameters<TranslateParams>,
    ) -> Result<String, String> {
        let res = self.resources()?;
    ...
    async fn process(&self, Parameters(p): Parameters<ProcessParams>) -> Result<String, String> {
        ...
        operations::process(
            ...
            PipelineJob {
                ...
                llm_custom_system_prompt: p.llm_custom_system_prompt,
                language: p.language,
                shader_effect: p.shader_effect,
                shader_stroke: p.shader_stroke,
                font_family: p.font_family,
    ...
    async fn update_text_block(
        ...
        let res = self.resources()?;
        let info = operations::update_text_block(
            res,
            TextBlockUpdate {
                document_index: p.index,
                text_block_index: p.text_block_index,
    ...
    async fn add_text_block(
        &self,
        Parameters(p): Parameters<CreateTextBlockParams>,
    ) -> Result<String, String> {
        let res = self.resources()?;
        let index = operations::add_text_block(
            res,
            CreateTextBlockJob {
                document_index: p.index,
                block: p.block,
    ...
    async fn remove_text_block(
        ...
        let res = self.resources()?;
        let remaining = operations::remove_text_block(
            res,
            RemoveTextBlockJob {
                document_index: p.index,
                text_block_index: p.text_block_index,
