...
use koharu_core::{
    Document, FontFaceInfo, SerializableDynamicImage, TextAlign, TextBlock, TextDirection,
...
impl RendererRuntime {
    ...
    pub(crate) fn available_fonts(&self) -> Result<Vec<FontFaceInfo>> {
        let fontbook = self
            .fontbook
            .lock()
    ...
    fn render_text_block(
        ...
        let color = text_block
            ...
            .or_else(|| {
                text_block.font_prediction.as_ref().map(|prediction| {
                    [
        ...
        let build_layout = |layout_box: LayoutBox, allow_expanded_overflow: bool| {
            let expanded_box = is_expanded_layout_box(layout_box, original_layout_box);
            ...
            let max_width = if layout_box.width.is_finite() && layout_box.width > 0.0 {
                layout_box.width * overflow
            } else {
                layout_box.width
            };
...
fn resolve_stroke_style(
    ...
    if let Some(prediction) = &block.font_prediction
        && prediction.stroke_width_px > 0.0
    {
        return Some(RenderStrokeOptions {
            ...
            width_px: prediction.stroke_width_px,
        });
    }
...
#[cfg(test)]
mod tests {
    use koharu_core::{TextAlign, TextBlock, TextDirection};
    use koharu_renderer::layout::{LayoutLine, WritingMode};

    use super::{
        EnglishLayoutBehavior, LayoutRun, align_layout_horizontally, apply_default_font_families,
    ...
    fn writing_mode_ignores_stale_rendered_direction() {
        ...
        let behavior = english_layout_behavior(&block, "HELLO", WritingMode::Horizontal);
        assert_eq!(behavior, EnglishLayoutBehavior::AutoExpand);
    }
}
