...
pub(crate) fn subscribe() -> broadcast::Receiver<PipelineProgress> {
...
pub(crate) async fn run_pipeline(
    ...
    let total_docs = match request.document_index {
        Some(_) => 1,
...
async fn run_pipeline_inner(
    ...
    let start_index = req.document_index.unwrap_or(0);
    let end_index = req.document_index.map(|i| i + 1).unwrap_or(total_docs);
    ...
    for (doc_ordinal, doc_index) in (start_index..end_index).enumerate() {
        for (step_ordinal, step) in PipelineStep::ALL.iter().enumerate() {
            ...
            let mut snapshot = store::read_doc(&res.state, doc_index).await?;

            match step {
                PipelineStep::Detect => res.vision.detect(&mut snapshot).await?,
                PipelineStep::Ocr => res.vision.ocr(&mut snapshot).await?,
                PipelineStep::Inpaint => res.vision.inpaint(&mut snapshot).await?,
            ...
            store::update_doc(&res.state, doc_index, snapshot, changed).await?;
        }
    }
