  ...
  return (
    <div className='ml-auto shrink-0'>
      <Popover>
        ...
        >
          <span
            className='flex h-7 w-14 items-center justify-between rounded-lg border border-border/50 bg-[var(--surface-well)] px-1.5 hover:text-foreground'
            title={`Koharu RAM: ${ramUsed} of ${ramTotal}`}
          >
        ...
        >
          ...
          <div className='divide-y divide-border/50 px-1.5 py-1 text-[11px]'>
            <ResourceRow icon={Cpu} label='Koharu CPU' value={cpu} detail='Process utilization' />
            <ResourceRow
              icon={MemoryStick}
              label='Koharu RAM'
              value={ram}
  ...
  return (
    <div className='grid grid-cols-[16px_minmax(0,1fr)_auto] items-center gap-x-2.5 px-2 py-2'>
      ...
      <span className='min-w-0'>
        <span className='block truncate text-foreground' title={label}>
          {label}
        </span>
        <span className='block truncate text-[9px] text-muted-foreground' title={detail}>
          {detail}
        </span>
      </span>
