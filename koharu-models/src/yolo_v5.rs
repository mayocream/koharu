...
const NUM_OUTPUTS: usize = NUM_CLASSES + 5;
...
impl Detect {
    fn load(vb: VarBuilder) -> Result<Self> {
        ...
        let anchors = vb.get((3, 3, 2), "anchors")?;
        Ok(Self {
    ...
    fn forward(&self, inputs: &[Tensor]) -> Result<(Tensor, Vec<Tensor>)> {
        ...
        for (idx, (conv, xs)) in self.convs.iter().zip(inputs).enumerate() {
            ...
            let xs = xs
                .reshape((b, NUM_ANCHORS, NUM_OUTPUTS, h, w))?
                .permute((0, 1, 3, 4, 2))?;
            let raw = xs.clone();
            ...
            outputs.push(y.reshape((b, NUM_ANCHORS * h * w, NUM_OUTPUTS))?);
            maps.push(raw);
        }
...
impl YOLOv5 {
    pub fn load(weights: impl AsRef<std::path::Path>, device: &Device) -> Result<Self> {
        ...
        let detect = Detect::load(vb.pp("model.24"))?;
    ...
    pub fn forward(&self, xs: &Tensor) -> Result<YOLOv5Output> {
        ...
        let (_, _, h10, w10) = x10.dims4()?;
        let x11 = x10.upsample_nearest2d(h10 * 2, w10 * 2)?;
        let x12 = Tensor::cat(&[&x11, &x6], 1)?;
        let x13 = self.l13.forward(&x12)?;
        let x14 = self.l14.forward(&x13)?;
        let (_, _, h14, w14) = x14.dims4()?;
        let x15 = x14.upsample_nearest2d(h14 * 2, w14 * 2)?;
        let x16 = Tensor::cat(&[&x15, &x4], 1)?;
        ...
        let det_features = [x17.clone(), x20.clone(), x23.clone()];
        let (predictions, raw_maps) = self.detect.forward(&det_features)?;
